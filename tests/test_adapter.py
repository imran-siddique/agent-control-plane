...
def test_adapter_blocks_unauthorized_action():
    ...
    assert blocked_call.function.name == "blocked_action"
    
    args = json.loads(blocked_call.function.arguments)
    assert args["original_tool"] == "write_file"
    assert "blocked" in args["reason"].lower()


def test_adapter_allows_authorized_action():
...
def test_adapter_mixed_permissions():
    ...
    assert len(tool_calls_result) == 2
    
    # First should be allowed
    assert tool_calls_result[0].function.name == "database_query"
    
    # Second should be blocked
    assert tool_calls_result[1].function.name == "blocked_action"
...
def test_custom_tool_mapping():
    ...
    tool_call = response.choices[0].message.tool_calls[0]
    assert tool_call.function.name == "my_custom_db_tool"
...
def test_on_block_callback():
    ...
    assert blocked_calls[0]["tool"] == "write_file"
    assert not blocked_calls[0]["result"]["success"]


def test_create_governed_client_convenience():
    ...
    tool_call = response.choices[0].message.tool_calls[0]
    assert tool_call.function.name == "database_query"


def test_adapter_statistics():
    ...
    assert "control_plane_audit" in stats
    assert len(stats["control_plane_audit"]) > 0


def test_no_tool_calls_response():
...
def test_pattern_matching_tool_names():
    ...
    for tool_name in tool_names:
        ...
        tool_call = response.choices[0].message.tool_calls[0]
        assert tool_call.function.name == tool_name, f"Pattern matching failed for {tool_name}"
